"""
Conversor SDMX — DEIE Mendoza  v1.1
Novedades:
- Soporte para columna intermedia "Unidad de Medida" (tablas de transporte, etc.)
- Unidad de medida por fila en lugar de global
- Fix API key: lee correctamente de st.secrets
- BASE_YEAR default vacío en lugar de 1988 fijo
"""
...
# Palabras que identifican una columna de unidad de medida
PALABRAS_UNIDAD = {'unidad de medida','unidades','unidad','medida','u. de medida','u.m.'}
...
# ─── Detección de columna de Unidad de Medida ─────────────────────────────────

def detectar_col_unidad(rows, fila_enc, col_rubro):
    """
    Busca si hay una columna de "Unidad de Medida" entre la columna de rubros
    y las columnas de datos. La busca en la fila del encabezado y en las 3 anteriores.
    Retorna el índice de esa columna o None.
    """
    filas_a_revisar = [rows[fila_enc]]
    for k in range(1, 4):
        if fila_enc - k >= 0:
            filas_a_revisar.append(rows[fila_enc - k])

    for fila in filas_a_revisar:
        for col_idx, v in enumerate(fila):
            if col_idx <= col_rubro:
                continue  # Solo columnas después del rubro
            norm = normalizar(str(v))
            if norm in PALABRAS_UNIDAD:
                return col_idx
    return None
...
def detectar_encabezado_en_fila(row, ignorar_cols=None):
    """
    Detecta períodos en la fila. ignorar_cols = set de índices a ignorar
    (ej: la columna de Unidad de Medida).
    """
    ignorar_cols = ignorar_cols or set()
    res = []
    for idx, v in enumerate(row):
        if idx in ignorar_cols:
            continue
...
def get_api_key():
    """Lee la API key de Streamlit secrets de forma robusta."""
    try:
        key = st.secrets["ANTHROPIC_API_KEY"]
        return key if key and key.startswith("sk-") else ""
    except Exception:
        return ""

def preview_a_texto(rows, max_filas=50):
    lines = []
...
{{
  ...
  "col_unidad_medida": número de columna (0-based) si hay una columna de unidad de medida por fila, o null,
  "freq": "M" | "A" | "Q" | "S",
  "unit_measure": "INDEX" | "ARS" | "KG" | "HL" | "TN" | "UNIDAD" | "MIXTO" | "otro",
  "base_year": año base si se menciona (ej: "1988"), o null,
  "col_tipos": {{
...
Notas importantes:
- col_tipos mapea índice de columna (0-based, como string) al tipo de medida
- Si hay una columna "Unidad de Medida" entre los rubros y los datos, indicala en col_unidad_medida
- En ese caso unit_measure debe ser "MIXTO" porque cada fila tiene su propia unidad
- Solo incluí en col_tipos las columnas de datos numéricos
- NO incluyas texto fuera del JSON."""
    ...
    try:
        ...
        if e.response.status_code == 401:
            return None, "API key inválida. Verificá el secret ANTHROPIC_API_KEY en Streamlit (Settings → Secrets)."
...
def parsear_datos(rows, fila_inicio, col_rubro, col_unidad, ref_area, unit_measure, base_year, mapeo_ia=None):
    """
    col_unidad: índice de la columna de Unidad de Medida por fila (o None si no existe).
    Cuando existe, cada registro usa la unidad de su propia fila en lugar del global.
    """
    registros   = []
    ignorar_enc = {col_unidad} if col_unidad is not None else set()

    bloques = []
    for i in range(fila_inicio, len(rows)):
        enc = detectar_encabezado_en_fila(rows[i], ignorar_cols=ignorar_enc)
        if enc:
    ...
    for b_idx, (fila_enc, enc_raw) in enumerate(bloques):
        ...
        for row in rows[fila_enc + 1 : fila_fin]:
            ...
            # Unidad de medida de esta fila (si hay columna específica)
            u_fila = unit_measure
            if col_unidad is not None and col_unidad < len(row):
                val_unidad = str(row[col_unidad]).strip()
                if val_unidad and val_unidad.lower() not in ('','nan','none'):
                    u_fila = val_unidad
            ...
            for col_idx, periodo_raw, freq, obs_msr in cols_info:
                ...
                registros.append({
                    ...
                    'UNIT_MEASURE':    u_fila if obs_msr != 'VAR_PCT' else 'PCT',
                    'BASE_YEAR':       base_year,
                })
...
if not usar_ia:
    st.warning("⚠️ No se encontró ANTHROPIC_API_KEY. "
               "Agregalo en Streamlit Cloud: Manage app → Settings → Secrets → "
               "ANTHROPIC_API_KEY = \"sk-ant-...\"")

# ── Paso 1 ────────────────────────────────────────────────────────────────────
st.markdown("""<div class="step-box">
  <p class="step-title">Paso 1 — Subí el archivo Excel</p>
  <p class="step-desc">XLS o XLSX de la DEIE — mensual, anual, trimestral, con o sin columna de unidades.</p>
...
st.markdown("""<div class="step-box">
  ...
  <p class="step-desc">Claude analiza la estructura y sugiere la configuración óptima.</p>
</div>""", unsafe_allow_html=True)
...
if usar_ia:
    if st.button("🤖 Analizar con IA", use_container_width=True):
        with st.spinner("Claude está analizando la estructura..."):
    ...
    st.info("ℹ️ Análisis IA no disponible — se usará detección automática.")

# ── Paso 4: Configuración ─────────────────────────────────────────────────────
st.markdown("""<div class="step-box">
  <p class="step-title">Paso 4 — Configuración</p>
  <p class="step-desc">Revisá y ajustá los parámetros (la IA los sugiere automáticamente).</p>
</div>""", unsafe_allow_html=True)
...
# Valores sugeridos
sugerido_fila   = int(mapeo_ia.get('fila_encabezado', primer_bloque + 1 if primer_bloque is not None else 1)) if mapeo_ia else (primer_bloque + 1 if primer_bloque is not None else 1)
sugerido_col    = int(mapeo_ia.get('col_rubros', 0))            if mapeo_ia else 0
sugerido_um     = mapeo_ia.get('unit_measure', 'INDEX')          if mapeo_ia else 'INDEX'
sugerido_by     = str(mapeo_ia.get('base_year') or '')           if mapeo_ia else ''
sugerido_cunidad= mapeo_ia.get('col_unidad_medida')              if mapeo_ia else None

col_a, col_b = st.columns(2)

with col_a:
    auto_msg = f"(IA: fila {sugerido_fila})" if mapeo_ia else \
               f"(auto: fila {primer_bloque+1})" if primer_bloque is not None else "(no detectada)"
    fila_enc_input = st.number_input(
        f"Fila desde donde empezar {auto_msg}",
        min_value=1, max_value=len(rows),
        value=int(sugerido_fila), step=1
    )
...
with col_b:
    ...
    col_rubro_label = st.selectbox(
        ...
        index=min(sugerido_col, len(opciones_col)-1)
    )
    col_rubro = opciones_col[col_rubro_label]

# Columna de Unidad de Medida
col_rubro_val = col_rubro

# Autodetectar columna de unidad si la IA no la sugirió
col_unidad_auto = detectar_col_unidad(rows, fila_inicio, col_rubro_val)
col_unidad_sugerida = sugerido_cunidad if sugerido_cunidad is not None else col_unidad_auto

opciones_unidad = {"(ninguna — unidad global)": None}
for i in range(min(len(rows[fila_ref]), 20)):
    if i == col_rubro:
        continue
    contenido = rows[fila_ref][i] or (rows[fila_ref+1][i] if fila_ref+1 < len(rows) else '')
    etiqueta  = f"Col {i+1}"
    if contenido and contenido not in ('','nan'):
        etiqueta += f" — {contenido[:25]}"
    opciones_unidad[etiqueta] = i

# Seleccionar por defecto la columna sugerida
idx_unidad_default = 0
for j, val in enumerate(opciones_unidad.values()):
    if val == col_unidad_sugerida:
        idx_unidad_default = j
        break

col_unidad_label = st.selectbox(
    "Columna de Unidad de Medida por fila (si existe)",
    list(opciones_unidad.keys()),
    index=idx_unidad_default,
    help="Seleccioná si hay una columna que indica la unidad de cada rubro (ej: 'Miles de pasajeros', 'Pesos', etc.)"
)
col_unidad = opciones_unidad[col_unidad_label]

if col_unidad is not None:
    st.markdown('<span class="tag-ok">✓ Unidad de medida por fila activada</span>', unsafe_allow_html=True)
...
with col_e:
    unit_measure = st.text_input("UNIT_MEASURE (global)", value=sugerido_um,
        help="Se usa cuando no hay columna de unidad por fila, o para registros VAR_PCT.")
with col_f:
    base_year = st.text_input("BASE_YEAR", value=sugerido_by,
        help="Año base del índice (ej: 1988). Dejá vacío si no aplica.")
...
if st.button("⚙️ Convertir a SDMX", type="primary", use_container_width=True):
    with st.spinner("Procesando..."):
        registros, error = parsear_datos(
            rows, fila_inicio, col_rubro, col_unidad,
            ref_area, unit_measure, base_year, mapeo_ia
        )
    ...
    if error:
        ...
        # Mostrar unidades detectadas si hay columna por fila
        if col_unidad is not None:
            unidades = df_out['UNIT_MEASURE'].dropna().unique()
            st.info(f"📐 Unidades detectadas: {', '.join(str(u) for u in unidades[:10])}")
...
st.markdown("---")
st.caption("Conversor SDMX v1.1.0 — DEIE Mendoza | Dirección de Estadísticas e Investigaciones Económicas")
