...
def clasificar_columnas(rows, fila_enc, enc_raw):
    ...
    Busca '%' o 'var' en:
      - hasta 3 filas ARRIBA del encabezado
      - la propia fila del encabezado
      - hasta 2 filas ABAJO del encabezado (donde a veces aparece "Var. %")
    Si no encuentra ningún %, asume que todas son INDEX.
    """
    ...
    todas_las_filas = filas_sup + [fila_row] + filas_inf
    ...
    # Si aún no encontró % explícito, buscar patrón en filas de contexto
    if not encontro_pct and len(enc_raw) >= 2:
        for fi in todas_las_filas:
...
st.markdown("---")
st.caption("Conversor SDMX v0.3.3 — DEIE Mendoza | Dirección de Estadísticas e Investigaciones Económicas")
