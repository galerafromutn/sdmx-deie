...
IGNORAR_RUBROS = ['fuente','nota','dato','promedio','rubros','nan','%','mes','año','anio','trimestre','semestre']
...
def detectar_periodo_en_celda(v):
    """
    Detecta si una celda representa un período temporal.
    Devuelve (periodo_raw, freq) o (None, None).
    periodo_raw puede ser:
      ('MES', N)   para meses
      ('TRIM', N)  para trimestres
      ('SEM', N)   para semestres
      'YYYY'       para años
      'YYYY/YYYY'  para rangos plurianuales
    """
    s = str(v).strip()
    ...
    # Rango de años "2022-2025" o "2022 - 2025"
    m = re.fullmatch(r'((19|20)\d{2})\s*[-–]\s*((19|20)\d{2})', s)
    if m:
        a1, a2 = m.group(1), m.group(3)
        return (a1 if int(a2)-int(a1) <= 1 else f"{a1}/{a2}"), ('A' if int(a2)-int(a1) <= 1 else 'P')

    # Trimestre
    m = re.search(r'(\d)[°º]?\s*(trim|°\s*trim)', normalizar(s))
    if not m:
        m = re.search(r'\bt\s*(\d)', normalizar(s))
    if m:
        num = m.group(1)
        return ('TRIM', int(num)), 'Q'

    # Semestre
    m = re.search(r'(\d)[°º]?\s*sem', normalizar(s))
    if m:
...
def detectar_encabezado_en_fila(row):
    ...
    for idx, v in enumerate(row):
        periodo, freq = detectar_periodo_en_celda(v)
        if periodo is not None:
            resultados.append((idx, periodo, freq))
    return resultados if len(resultados) >= 2 else []
...
def resolver_time_period(periodo_raw, freq, anio_contexto):
    if isinstance(periodo_raw, tuple):
        tipo, num = periodo_raw
        if tipo == 'MES':
            return (f"{anio_contexto}-{num:02d}", 'M') if anio_contexto else (None, freq)
        if tipo == 'TRIM':
            return (f"{anio_contexto}-Q{num}", 'Q') if anio_contexto else (None, freq)
        if tipo == 'SEM':
            return (f"{anio_contexto}-S{num}", 'S') if anio_contexto else (None, freq)
    return str(periodo_raw), freq

def parsear_datos(rows, fila_encabezado, col_rubro, ref_area, unit_measure, base_year):
    """
    Genera registros SDMX correctos:
    Cada observación tiene su propia fila con OBS_MSR = INDEX o VAR_PCT.
    Una fila del Excel genera DOS filas SDMX (una por medida).
    """
    registros = []
    anio_contexto = None
    ...
    # Clasificar columnas como INDEX o VAR_PCT
    # Mirar fila del encabezado y la fila anterior
    fila_sup = rows[fila_encabezado - 1] if fila_encabezado > 0 else []
    ...
    for col_idx, periodo_raw, freq in enc_raw:
        ...
        celda_sup = normalizar(str(fila_sup[col_idx]) if col_idx < len(fila_sup) else '')
        if '%' in celda_enc or 'var' in celda_enc or '%' in celda_sup or 'var' in celda_sup:
            col_tipo[col_idx] = 'VAR_PCT'
        else:
            col_tipo[col_idx] = 'INDEX'

    # Si todas quedaron INDEX, intentar alternado (patrón DEIE: Índice, %, Índice, %)
    if all(t == 'INDEX' for t in col_tipo.values()) and len(enc_raw) >= 2:
        for i, (col_idx, _, _) in enumerate(enc_raw):
            col_tipo[col_idx] = 'INDEX' if i % 2 == 0 else 'VAR_PCT'
    ...
    for row in rows[fila_encabezado + 1:]:
        ...
        for v in row:
            ...
            if m:
                anio_contexto = int(m.group(0))
                break

        # Ignorar filas de metadatos o vacías
        if not col0 or col0.lower() in ('','nan','none'):
            continue
        norm0 = normalizar(col0)
        if any(norm0.startswith(p) for p in IGNORAR_RUBROS):
            continue
        if re.fullmatch(r'(19|20)\d{2}', col0.strip()):
            continue
        ...
        for col_idx, periodo_raw, freq, obs_msr in cols_info:
            ...
            time_period, freq_final = resolver_time_period(periodo_raw, freq, anio_contexto)
...
CREATE TABLE IF NOT EXISTS public.{nombre_tabla} (
    ...
    "OBS_MSR"         VARCHAR(10)   NOT NULL,          -- INDEX = valor índice | VAR_PCT = variacion %
...
st.markdown("""<div class="step-box">
  ...
  <p class="step-desc">Elegí la hoja que tenga todos los rubros juntos.</p>
</div>""", unsafe_allow_html=True)
...
with col_b:
    ...
    col_rubro_label = st.selectbox(
        ...
        help="La columna que contiene los nombres de cada rubro."
    )
...
st.markdown("---")
st.caption("Conversor SDMX v0.3.0 — DEIE Mendoza | Dirección de Estadísticas e Investigaciones Económicas")
