...
def es_rubro_valido(texto):
    """True si el texto es un nombre de rubro real (no nota, gráfico, cuadro, etc.)"""
...
def clasificar_columnas(rows, fila_enc, enc_raw):
    """
    Determina si cada columna es INDEX o VAR_PCT mirando hasta 3 filas arriba.
    Si no encuentra ningún %, asume que todas son INDEX (ej: tabla anual sin variación).
    Si encuentra patrón alternado (Índice/%), lo aplica.
    """
    ...
    for col_idx, _, _ in enc_raw:
        celdas = [str(fila_row[col_idx]) if col_idx < len(fila_row) else '']
        celdas += [str(f[col_idx]) if col_idx < len(f) else '' for f in filas_sup]
    ...
    # Si no encontró % explícito, buscar patrón alternado en filas superiores
    if not encontro_pct and len(enc_raw) >= 2:
        for fi in filas_sup:
            if any('%' in normalizar(str(v)) for v in fi):
...
def parsear_datos(rows, fila_inicio, col_rubro, ref_area, unit_measure, base_year):
    """
    Procesa TODOS los bloques de la hoja independientemente.
    Un bloque = fila de encabezado + filas de datos hasta el próximo encabezado.
    Esto resuelve el problema de hojas con múltiples tablas (ej: 2026 arriba con 3 meses,
    2022-2025 abajo con 12 meses, 2018-2021 más abajo, etc.)
    """
    registros = []

    # 1. Encontrar todas las filas que son encabezados de bloque
    bloques = []  # lista de (fila_idx, enc_raw)
    ...
    # 2. Procesar cada bloque hasta el inicio del siguiente
    for b_idx, (fila_enc, enc_raw) in enumerate(bloques):
        ...
        # Buscar año de contexto en las filas anteriores a este bloque
        anio_ctx = None
...
st.markdown("---")
st.caption("Conversor SDMX v0.3.2 — DEIE Mendoza | Dirección de Estadísticas e Investigaciones Económicas")
